...
export const DocumentsLibrary: React.FC = () => {
  ...
  const handleDocumentClick = (doc: Document) => {
      // Auto-detect type if marked as 'link' or unspecified
      const videoId = getYoutubeId(doc.url);
      const isPdf = doc.url.toLowerCase().endsWith('.pdf');
      
      let effectiveType = doc.type;
      
      // Upgrade 'link' to specific types if detected
      ...
      if (effectiveType === 'link') {
          ...
          // Links are marked complete immediately upon click for simplicity
          updateProgress(doc.id, true, 0);
      } else {
          // Open viewer with detected type
...
const DocItem: React.FC<{ doc: Document; completed: boolean; onClick: () => void }> = ({ doc, completed, onClick }) => {
    // Helper to check what icon to show, checking logic similar to click handler for better UI feedback
    const videoId = getYoutubeId(doc.url);
    const isPdf = doc.url.toLowerCase().endsWith('.pdf');
    let effectiveType = doc.type;
...
// Helper for Youtube ID with Shorts support
...
// Helper to construct robust embed URL
const getEmbedUrl = (docUrl: string) => {
    try {
        ...
        if (docUrl.includes('/embed/')) {
            ...
            // Force nocookie domain for better privacy/compatibility
            return url.toString().replace('youtube.com', 'youtube-nocookie.com');
        }
        
        // Case 2: Standard watch URL -> Convert to embed
        const videoId = getYoutubeId(docUrl);
        if (videoId) {
            return `https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&origin=${origin}&rel=0`;
        }
    } catch (e) {
                             ...
                             <>
                                ...
                                <div className="absolute bottom-6 left-0 w-full text-center pointer-events-none z-20">
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noreferrer" 
                                        className="pointer-events-auto inline-block bg-gray-900 bg-opacity-75 text-white px-4 py-2 rounded-full text-xs hover:bg-opacity-100 transition shadow-lg border border-gray-700"
                                    >
                                        Nếu lỗi, bấm vào đây để xem trên YouTube ↗
                                    </a>
                                </div>
                             </>
                         ) : (
